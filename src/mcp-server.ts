...
import { autoGeneratedMcpTools, getMcpToolNames } from './schemas/mcp-tools.js';
...
/** Type guard for a single move entry with string source/destination paths */
function isMoveEntry(value: unknown): value is { source: string; destination: string } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'source' in value &&
      ...
      if (methodName === 'moveFile') {
        if (typeof args === 'object' && args !== null && !Array.isArray(args)) {
          ...
          if (
            ...
            result = await markmv.moveFile(sourcePath, destinationPath, options);
          } else {
            throw new Error(
        ...
        if (typeof args === 'object' && args !== null && !Array.isArray(args)) {
          ...
          if (
            ...
            throw new Error(
              'Invalid parameters for moveFiles: moves must be an array of { source, destination } pairs'
            );
          }
        } else {
