...
Advanced concepts here.`,
      ...
      const joinedContent = await fs.readFile(
        join(testDir, "joined.md"),
        "utf8",
      );
      expect(joinedContent).toContain(
        'title: "Advanced & Basics & Introduction"',
      );
...
# Content 2`,
      ...
      const joinedContent = await fs.readFile(
        join(testDir, "conflict-joined.md"),
        "utf8",
      );
    ...
    it("should handle duplicate headers", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.warnings.some((w) => w.includes("duplicate-headers"))).toBe(
        true,
      );
    });
    ...
    it("should handle missing files gracefully", async () => {
      ...
      const joinedContent = await fs.readFile(
        join(testDir, "partial-joined.md"),
        "utf8",
      );
...
Content of section 2.`,
      ...
      const joinedContent = await fs.readFile(
        join(testDir, "structured-joined.md"),
        "utf8",
      );
