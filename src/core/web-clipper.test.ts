...
vi.mock("jsdom", () => ({
  JSDOM: vi
    .fn()
    .mockImplementation((_html: string, _options: { url: string }) => ({
      window: {
        document: {
          title: "Test Article",
          body: {
      ...
      querySelector: vi.fn().mockImplementation((selector: string) => {
        ...
        if (selector === ".article-content") {
          return {
            ...baseElement,
            innerHTML:
              "<h1>Manual Extraction</h1><p>This content was extracted manually.</p>",
          };
        }
        if (selector === "h1") {
          return {
...
describe("WebClipper", () => {
  ...
  describe("URL fetching", () => {
    ...
    it("should handle HTTP errors", async () => {
      ...
      await expect(
        clipper.clip("https://example.com/nonexistent"),
    ...
    it("should handle network errors", async () => {
      ...
      await expect(
        clipper.clip("https://example.com/unreachable"),
    ...
    it("should respect timeout", async () => {
      ...
      await expect(
        timeoutClipper.clip("https://slow.example.com"),
      ).rejects.toThrow();
    });
  ...
  describe("Content extraction strategies", () => {
    it("should extract content using readability strategy", async () => {
      ...
      const result = await readabilityClipper.clip(
        "https://example.com/article",
      );
    ...
    it("should extract content using structured strategy", async () => {
      ...
      const result = await structuredClipper.clip(
  ...
  describe("Link and image processing", () => {
    ...
    it("should extract images with metadata", async () => {
      ...
      expect(result.images[1].originalUrl).toBe(
        "https://external.com/image2.png",
      );
      expect(result.images[2].originalUrl).toBe(
        "https://example.com/relative-image.gif",
      );
    });
    ...
    it("should handle different image strategies", async () => {
      ...
      const mockResponse = {
        ...
        text: vi
          .fn()
          .mockResolvedValue(
            '<html><body><article><img src="test.jpg" /></article></body></html>',
          ),
      };
      vi.mocked(fetch).mockResolvedValue(mockResponse as unknown as Response);

      const skipResult = await skipImagesClipper.clip(
        "https://example.com/images",
      );
      const linkResult = await linkOnlyClipper.clip(
        "https://example.com/images",
      );
  ...
  describe("Frontmatter generation", () => {
    it("should generate frontmatter when enabled", async () => {
      ...
      const result = await frontmatterClipper.clip(
        "https://example.com/frontmatter-test",
      );
    ...
    it("should skip frontmatter when disabled", async () => {
      ...
      const result = await noFrontmatterClipper.clip(
        "https://example.com/no-frontmatter",
      );
  ...
  describe("Error handling", () => {
    it("should handle Readability parsing failures", async () => {
      ...
      const mockResponse = {
        ...
        text: vi
          .fn()
          .mockResolvedValue(
            "<html><body><p>Unparseable content</p></body></html>",
          ),
      };
      vi.mocked(fetch).mockResolvedValue(mockResponse as unknown as Response);

      await expect(
        failingClipper.clip("https://example.com/unparseable"),
      ).rejects.toThrow("Could not extract article content using Readability");
    });
    ...
    it("should handle selector not found in manual strategy", async () => {
      ...
      await expect(
        manualClipper.clip("https://example.com/no-selectors"),
      ).rejects.toThrow("Could not find content with specified selectors");
    });
    ...
    it("should handle missing body element in full strategy", async () => {
      ...
      await expect(
        fullClipper.clip("https://example.com/no-body"),
      ).rejects.toThrow("Could not find body element");
    });
