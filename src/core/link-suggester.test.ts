...
describe('link suggester', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(join(vaultRoot, 'guides', 'getting-started.md'), '# Getting Started\n');
    await writeFile(join(vaultRoot, 'guides', 'testing-strategies.md'), '# Testing\n');
    await writeFile(join(vaultRoot, 'reference', 'magic-numbers.md'), '# Magic\n');
  });
