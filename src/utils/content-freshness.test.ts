...
describe("ContentFreshnessDetector", () => {
  ...
  describe("Configuration", () => {
    ...
    it("should use default freshness configuration", () => {
      ...
      expect(DEFAULT_FRESHNESS_CONFIG.defaultThreshold).toBe(
        2 * 365 * 24 * 60 * 60 * 1000,
      );
      expect(DEFAULT_FRESHNESS_CONFIG.stalePatterns).toContain("deprecated");
      expect(
        DEFAULT_FRESHNESS_CONFIG.domainThresholds["firebase.google.com"],
      ).toBe(1 * 365 * 24 * 60 * 60 * 1000);
    });
  });
  ...
  describe("Content Analysis", () => {
    ...
    it("should apply domain-specific thresholds", async () => {
      ...
      const dateEightMonthsAgo = new Date(
    ...
    it("should detect multiple stale patterns", async () => {
      const response: ResponseInfo = {
        ...
        headers: {},
        content:
          "This page has moved and the content is deprecated. This is legacy documentation.",
  ...
  describe("Cache Management", () => {
    ...
    it("should handle cache errors gracefully", async () => {
      ...
      const result = await invalidDetector.analyzeContentFreshness(
        url,
        response,
      );
      expect(result.contentHash).toBeDefined();
  ...
  describe("Performance", () => {
    it("should handle large content efficiently", async () => {
      const largeContent =
        "Large content section ".repeat(10000) + "deprecated";
      const response: ResponseInfo = {
        status: 200,
        headers: {},
