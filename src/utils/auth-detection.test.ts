 ...
 *
 * @file Tests for AuthDetector functionality and configuration
 */
...
describe("AuthDetector", () => {
  ...
  describe("Configuration", () => {
    ...
    it("should support custom credentials", () => {
      ...
      expect(
        credentialsDetector.shouldAttemptAuth("https://api.github.com/user"),
      ).toBe(true);
      expect(
        credentialsDetector.shouldAttemptAuth("https://api.example.com/data"),
      ).toBe(true);
      expect(
        credentialsDetector.shouldAttemptAuth("https://other.example.com/data"),
      ).toBe(false);
    });
  });
  ...
  describe("Domain-based Authentication Detection", () => {
    it("should detect Firebase Console URLs as auth-required", () => {
      const url =
        "https://console.firebase.google.com/project/my-project/settings";
  ...
  describe("Response Analysis", () => {
    ...
    it("should detect redirects to authentication pages", () => {
      ...
      const authInfo = authDetector.analyzeAuth(originalUrl, mockResponse, [
        finalUrl,
      ]);
  ...
  describe("Authentication Headers", () => {
    it("should provide auth headers for configured domains", () => {
      ...
      const githubHeaders = credentialsDetector.getAuthHeaders(
      ...
      const exampleHeaders = credentialsDetector.getAuthHeaders(
    ...
    it("should provide custom headers for configured domains", () => {
      ...
      const headers = headersDetector.getAuthHeaders(
        "https://api.custom.com/v1/data",
      );
    ...
    it("should return empty headers for unconfigured domains", () => {
      const headers = authDetector.getAuthHeaders(
    ...
    it("should support wildcard credentials", () => {
      ...
      const headers = wildcardDetector.getAuthHeaders(
        "https://any.example.com/api",
      );
      expect(headers).toEqual({
  ...
  describe("Authentication Attempt Detection", () => {
    ...
    it("should attempt auth for configured custom header domains", () => {
      ...
      expect(
        headersDetector.shouldAttemptAuth("https://api.custom.com/data"),
      ).toBe(true);
      expect(
        headersDetector.shouldAttemptAuth("https://other.example.com/data"),
      ).toBe(false);
    });
    ...
    it("should handle subdomain matching", () => {
      ...
      expect(
        credentialsDetector.shouldAttemptAuth(
  ...
  describe("Redirect Analysis", () => {
    it("should detect auth redirects by URL patterns", () => {
      ...
      for (const testCase of testCases) {
        ...
        const authInfo = authDetector.analyzeAuth(
    ...
    it("should detect auth parameters in URLs", () => {
      ...
      Object.defineProperty(mockResponse, "url", {
        value:
          "https://example.com/dashboard?login=required&redirect_uri=callback",
        writable: false,
      });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle disabled detection", () => {
      const disabledDetector = new AuthDetector({ enabled: false });
      const authInfo = disabledDetector.analyzeAuth(
  ...
  describe("Default Configuration", () => {
    it("should have comprehensive default auth domain patterns", () => {
      expect(DEFAULT_AUTH_CONFIG.authDomainPatterns).toContain(
    ...
    it("should have comprehensive default redirect patterns", () => {
      expect(DEFAULT_AUTH_CONFIG.authRedirectPatterns).toContain(
        "accounts.google.com",
      );
      expect(DEFAULT_AUTH_CONFIG.authRedirectPatterns).toContain(
