...
describe("GitUtils", () => {
  ...
  describe("Repository Detection", () => {
    ...
    it("should throw error when getting root of non-git directory", () => {
      ...
      expect(() => gitUtils.getRepositoryRoot()).toThrow(
  ...
  describe("File Changes Detection", () => {
    ...
    it("should handle renamed files", () => {
      ...
      expect(toPosix(rename.previousPath ?? "")).toBe(
        `${resolvedRoot}/old-name.md`,
      );
    });
  ...
  describe("Combined Operations", () => {
    ...
    it("should deduplicate files in getAllModifiedFiles", () => {
      ...
      expect(result).toHaveLength(1);
      expect(toPosix(result[0]?.path ?? "")).toBe(
        `${resolvedRoot}/same-file.md`,
      );
    });
  });
