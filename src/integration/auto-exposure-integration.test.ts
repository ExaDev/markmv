...
import { autoGeneratedApiRoutes, getApiRouteByPath } from '../schemas/api-routes.js';
...
async function connectMcpClient(): Promise<Client> {
  ...
  const client = new Client({ name: 'auto-exposure-integration-test-client', version: '1.0.0' });
    ...
    it('should handle concurrent operations efficiently', async () => {
      ...
      const promises = Array(concurrentCount)
        ...
        .map(async (_, index) => {
          ...
          // Execute function
          const result = await testAutoExposure(`${testInput} ${String(index)}`);
