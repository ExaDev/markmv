...
import {
  ...
  getApiRouteByPath,
} from "../schemas/api-routes.js";
import { validateInput, validateOutput } from "../schemas/validators.js";

/**
 * Connect a real MCP Client to a real createMcpServer() over an in-memory transport pair, so tool
 * discovery goes through the actual protocol rather than a hand-built tool array.
 */
async function connectMcpClient(): Promise<Client> {
  const server = createMcpServer();
  const client = new Client({
    name: "auto-exposure-integration-test-client",
    version: "1.0.0",
  });
...
describe("Auto-Exposure Pattern Integration", () => {
  ...
  beforeAll(async () => {
    testDir = join(tmpdir(), `markmv-integration-${String(Date.now())}`);
  ...
  describe("End-to-End Workflow", () => {
    ...
    it("should expose testAutoExposure function through all channels", async () => {
      ...
      const mcpTool = tools.find((tool) => tool.name === "test_auto_exposure");
      expect(mcpTool).toBeDefined();
      expect(mcpTool?.name).toBe("test_auto_exposure");
      expect(mcpTool?.description).toContain(
        "Test function to demonstrate auto-exposure",
      );
    ...
    it("should maintain consistency across all exposure channels", async () => {
      ...
      const mcpInputSchema =
        mcpTool?.inputSchema as unknown as InputPropertySchema;
      const apiInputSchema =
        apiRoute?.inputSchema as unknown as InputPropertySchema;

      expect(mcpInputSchema.properties.input.type).toBe(
        apiInputSchema.properties.input.type,
      );
  ...
  describe("Schema Validation Integration", () => {
    it("should validate complex input scenarios", () => {
      const testCases = [
        ...
        { input: "🚀 Unicode test! @#$%", shouldPass: true },
      ...
      testCases.forEach(({ input, shouldPass }, _index) => {
        // input narrows to string | number | null here (the ternary below already excludes undefined); of those, only `null` is typeof 'object', and that's excluded too, so this never took the removed `typeof input === 'object' && input !== null ? input : ...` branch -- it always fell through to `{}` for the same cases this reaches now.
  ...
  describe("Error Handling Integration", () => {
    ...
    it("should provide meaningful error messages", () => {
      ...
      expect(
        result.errors.some(
          (error) => error.includes("type") || error.includes("string"),
        ),
      ).toBe(true);
    });
  ...
  describe("Performance Integration", () => {
    it("should handle concurrent operations efficiently", async () => {
      ...
      const promises = Array(concurrentCount)
        ...
        .map(async (_, index) => {
          ...
          const inputValidation = validateInput("testAutoExposure", {
            input: `${testInput} ${String(index)}`,
          });
          expect(inputValidation.valid).toBe(true);

          // Execute function
          const result = await testAutoExposure(
            `${testInput} ${String(index)}`,
          );
      ...
      results.forEach((result, index) => {
        expect(result.message).toBe(`Echo: ${testInput} ${String(index)}`);
  ...
  describe("Complete System Integration", () => {
    it("should demonstrate zero-config auto-exposure", async () => {
      ...
      const testTool = mcpTools.find(
        (tool) => tool.name === "test_auto_exposure",
      );
      ...
      const testRoute = apiRoutes.find(
        (route) => route.path === "/api/test-auto-exposure",
      );
      expect(testRoute).toBeDefined();

      // 4. Should have working validation
      expect(validateInput("testAutoExposure", { input: "test" }).valid).toBe(
        true,
      );
