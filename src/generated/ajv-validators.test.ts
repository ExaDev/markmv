/**
 * @file Tests for auto-generated AJV validators Verifies that JSON Schema validation works
 *   correctly for all auto-exposed methods
 */
...
describe('Auto-Generated AJV Validators', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should provide meaningful error messages', () => {
      ...
      expect(result.errors.length).toBeGreaterThan(0);
      expect(
        result.errors.some((error) => error.includes('string') || error.includes('type'))
      ).toBe(true);
    });
  });
  ...
  describe('Performance', () => {
    ...
    it('should handle concurrent validations', async () => {
      ...
      const promises = Array(100)
        .fill(0)
        .map(() => Promise.resolve(validateInput('testAutoExposure', validInput)));
