...
export async function splitCommand(
  ...
  if (options.splitLines) {
    try {
      ...
      if (splitLines.length === 0) {
        console.error(
          "❌ Invalid split lines format. Use comma-separated numbers like: 10,25,50",
        );
        process.exit(1);
      }
    } catch (error) {
  ...
  if (options.verbose) {
    ...
    if (splitOptions.strategy === "headers") {
      console.log(
        `📋 Split on header level: ${String(splitOptions.headerLevel)}`,
      );
    }
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log(
        `📄 Created ${String(result.createdFiles.length)} new file(s):`,
      );
      for (const file of result.createdFiles) {
        console.log(`  + ${file}`);
      }
    ...
    if (!options.dryRun) {
      ...
      if (splitOptions.strategy === "headers") {
        console.log(
          "  • Use --header-level to control which headers trigger splits",
        );
      }
      if (splitOptions.strategy === "size") {
        console.log(
          "  • Use --max-size to adjust the maximum file size (in KB)",
        );
      }
