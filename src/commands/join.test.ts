...
describe("Join Command", () => {
  ...
  beforeEach(() => {
    testDir = join(
      tmpdir(),
      `markmv-join-test-${String(Date.now())}-${Math.random().toString(36).slice(2, 11)}`,
    );
  ...
  describe("Argument Validation", () => {
    it("should exit with error when no files provided", async () => {
      await expect(joinCommand([], {})).rejects.toThrow(
        "Process exit called with code 1",
      );

      expect(mockConsoleError).toHaveBeenCalledWith(
        "❌ No files provided to join",
      );
  ...
  describe("File Processing", () => {
    it("should exit with error when only one file provided", async () => {
      ...
      await expect(
        joinCommand([file1], { dryRun: true, verbose: true }),
  ...
  describe("Order Strategies", () => {
    it("should handle alphabetical order strategy", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining(
          "🔗 Joining 2 files using alphabetical strategy",
        ),
      );
    });
    ...
    it("should handle chronological order strategy", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining(
          "🔗 Joining 2 files using chronological strategy",
        ),
      );
    });
  });
  ...
  describe("Verbose Output", () => {
    it("should show detailed information in verbose mode", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining("📁 Input files:"),
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
