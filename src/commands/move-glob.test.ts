...
describe("Glob Pattern Support in Move Command", () => {
  ...
  describe("Glob expansion patterns", () => {
    ...
    it("should ignore common directories", async () => {
      ...
      await writeFile(
        join(testDir, "node_modules", "bad.md"),
        "# Should be ignored",
      );
  ...
  describe("Real-world scenarios", () => {
    it("should handle typical documentation structure", async () => {
      ...
      await writeFile(
        join(testDir, "docs", "api", "overview.md"),
        "# API Overview",
      );
      await writeFile(
        join(testDir, "docs", "guides", "getting-started.md"),
        "# Getting Started",
      );
      ...
      const allMd = await glob(join(testDir, "**/*.md").replace(/\\/g, "/"), {
        absolute: true,
      });
      const docsOnly = await glob(
        join(testDir, "docs/**/*.md").replace(/\\/g, "/"),
        {
          absolute: true,
        },
      );
      const rootOnly = await glob(join(testDir, "*.md").replace(/\\/g, "/"), {
        absolute: true,
      });
