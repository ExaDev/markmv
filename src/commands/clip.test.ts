...
const mockClip = vi.hoisted(() =>
  vi.fn<(url: string) => Promise<ClipResult>>(),
);
...
function isClipCommandJsonOutput(
  value: unknown,
): value is ClipCommandJsonOutput {
  if (typeof value !== "object" || value === null) return false;
  if (!("clippedUrls" in value) || !("generatedFiles" in value)) return false;
  return (
    Array.isArray(value.clippedUrls) && Array.isArray(value.generatedFiles)
  );
}
...
describe("Clip Command", () => {
  ...
  describe("Basic functionality", () => {
    it("should require at least one URL", async () => {
      ...
      expect(
        errors.some((error) =>
          error.includes("At least one URL must be specified"),
        ),
      ).toBe(true);
    });
    ...
    it("should process a single URL successfully", async () => {
      ...
      expect(logs.some((log) => log.includes("Successfully clipped: 1"))).toBe(
        true,
      );
    });
    ...
    it("should handle dry run mode", async () => {
      ...
      expect(
        logs.some((log) =>
          log.includes("Dry run - no files were actually created"),
        ),
      ).toBe(true);
  ...
  describe("Batch processing", () => {
    it("should process multiple URLs from a file", async () => {
      ...
      expect(logs.some((log) => log.includes("Successfully clipped: 3"))).toBe(
        true,
      );
    });
    ...
    it("should handle mixed valid and invalid URLs in batch mode", async () => {
      ...
      expect(mockClip).toHaveBeenCalledWith(
        "https://example.com/another-valid",
      );

      expect(logs.some((log) => log.includes("Successfully clipped: 2"))).toBe(
        true,
      );
    });
  });
  ...
  describe("Option parsing", () => {
    ...
    it("should handle invalid JSON headers gracefully", async () => {
      ...
      expect(
        errors.some((error) =>
          error.includes("Invalid JSON format for headers"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Error handling", () => {
    it("should handle clipping failures gracefully", async () => {
      ...
      expect(logs.some((log) => log.includes("Failed to fetch URL"))).toBe(
        true,
      );
    });
    ...
    it("should continue processing other URLs when one fails", async () => {
      ...
      try {
        await clipCommand(
          ["https://fail.example", "https://example.com/success"],
          {
            outputDir: testDir,
            verbose: true,
          },
  ...
  describe("Output formatting", () => {
    it("should format results with comprehensive summary", async () => {
      ...
      try {
        await clipCommand(
          ["https://example.com/success1", "https://example.com/fail"],
          {
            outputDir: testDir,
            verbose: true,
          },
