...
async function createTreeFixture(testDir: string): Promise<void> {
  ...
  await writeFile(join(testDir, 'docs', 'orphan.md'), '# Orphan\n\nNobody links here.\n');
...
describe('Tree Command', () => {
  ...
  describe('buildFileTree', () => {
    ...
    it('nests directories and files, ordering directories first then files, alphabetically', () => {
      ...
      expect(tree.directories.map((dir) => dir.name)).toEqual(['assets', 'docs']);
      expect(tree.files.map((file) => file.name)).toEqual(['README.md', 'alpha.md', 'zeta.md']);

      const docs = tree.directories.find((dir) => dir.name === 'docs');
      expect(docs?.directories.map((dir) => dir.name)).toEqual(['nested']);
      expect(docs?.files.map((file) => file.name)).toEqual(['guide.md', 'orphan.md']);
    });
  ...
  describe('renderTreeAscii', () => {
    const annotated: ScannedMarkdownFile[] = [
      ...
      { ...scanned('docs/orphan.md'), wordCount: 5, linkCount: 0, inboundLinkCount: 0 },
    ];
    ...
    it('marks max-depth-truncated directories with an ellipsis', () => {
      const rendering = renderTreeAscii(buildFileTree(annotated, 1), 'test-root');
    ...
    it('uses singular word and link labels for counts of one', () => {
      const single: ScannedMarkdownFile[] = [{ ...scanned('solo.md'), wordCount: 1, linkCount: 1 }];

      const rendering = renderTreeAscii(buildFileTree(single), 'test-root');

      expect(rendering).toBe(['test-root', '└── solo.md (1 word, 1 link)'].join('\n'));
    });
  });
  ...
  describe('treeCommand', () => {
    ...
    it('renders the ASCII tree and statistics block by default', async () => {
      ...
      expect(logs.some((log) => log === '│   ├── guide.md (9 words, 2 links) [1 broken]')).toBe(
        true
      );
      expect(logs.some((log) => log === '│   └── orphan.md (5 words, 0 links) [orphan]')).toBe(
        true
      );
      expect(logs.some((log) => log === '└── README.md (8 words, 2 links)')).toBe(true);
      expect(logs.some((log) => log.includes('📊 Markdown Tree Statistics'))).toBe(true);
      expect(logs.some((log) => log.includes('📁 Markdown files: 3'))).toBe(true);
      expect(logs.some((log) => log.includes('📝 Total words: 22'))).toBe(true);
      expect(logs.some((log) => log.includes('🔗 Internal links: 3'))).toBe(true);
      expect(logs.some((log) => log.includes('🌐 External links: 1'))).toBe(true);
      expect(logs.some((log) => log.includes('❌ Broken internal links: 1'))).toBe(true);
      expect(logs.some((log) => log.includes('🔍 Orphaned files: 1'))).toBe(true);
    });
    ...
    it('serialises the whole tree and statistics with a stable key order in json format', async () => {
      ...
      expect(docs?.files.map((file) => file.name)).toEqual(['guide.md', 'orphan.md']);
      expect(docs?.files.find((file) => file.name === 'orphan.md')?.orphaned).toBe(true);
      expect(docs?.files.find((file) => file.name === 'guide.md')?.orphaned).toBe(false);
    ...
    it('fails loudly for a path that does not exist', async () => {
      ...
      expect(errors.some((error) => error.includes('Tree analysis failed'))).toBe(true);
    });
    ...
    it('rejects an invalid format option', async () => {
      ...
      expect(exitCode).toBe(1);
      expect(
        errors.some((error) => error.includes("Invalid format: svg. Must be 'ascii' or 'json'"))
      ).toBe(true);
    });
    ...
    it('accepts a single markdown file as the scan target', async () => {
      ...
      expect(exitCode).toBe(0);
      expect(logs.some((log) => log === '└── README.md (8 words, 2 links) [orphan]')).toBe(true);
      expect(logs.some((log) => log.includes('📁 Markdown files: 1'))).toBe(true);
    ...
    it('treats the --json flag as an alias for --format json', async () => {
      ...
      expect(exitCode).toBe(0);
      const parsed = JSON.parse(logs.join('\n')) as { statistics: Record<string, number> };
    ...
    it('prints scan progress in verbose mode', async () => {
      ...
      expect(exitCode).toBe(0);
      expect(logs.some((log) => log.includes('Analysing markdown tree:'))).toBe(true);
      expect(logs.some((log) => log.includes(`Parsing: ${resolve(testDir, 'README.md')}`))).toBe(
        true
      );
      expect(logs.some((log) => log.includes('Skipping excluded directory:'))).toBe(true);
    });
  });
});
