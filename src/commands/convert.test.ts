...
`;
      ...
      expect(logs.some((log) => log.includes("Starting link conversion"))).toBe(
        true,
      );
...
`;
      ...
      expect(getExitCode()).toBe(0);
      expect(logs.some((log) => log.includes("No changes were needed"))).toBe(
        true,
      );
    });
    ...
    it("should validate conversion options", async () => {
      ...
      expect(
        errors.some((error) =>
          error.includes("At least one conversion option must be specified"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle invalid path resolution option", async () => {
      ...
      expect(getExitCode()).toBe(1);
      expect(
        errors.some((error) => error.includes("Invalid path resolution type")),
      ).toBe(true);
    });
    ...
    it("should handle invalid link style option", async () => {
      ...
      try {
        await convertCommand([testFile], {
          linkStyle: "invalid" as
            "markdown" | "claude" | "combined" | "wikilink",
        });
      } finally {
        restore();
        console.error = originalError;
      ...
      expect(getExitCode()).toBe(1);
      expect(errors.some((error) => error.includes("Invalid link style"))).toBe(
        true,
      );
    });
  });
  ...
  describe("File pattern expansion", () => {
    ...
    it("should handle directory patterns with recursive option", async () => {
      ...
      expect(getExitCode()).toBe(0);
      expect(
        logs.some((log) => log.includes("markdown files to process")),
      ).toBe(true);
    });
...
`;
      ...
      expect(
        logs.some((log) =>
          log.includes("Path resolution: converted to relative"),
        ),
      ).toBe(true);
      expect(
        logs.some((log) => log.includes("Link style: converted to wikilink")),
      ).toBe(true);
      expect(
        logs.some((log) =>
...
`;
      ...
      expect(
        logs.some((log) => log.includes("Link style: converted to combined")),
      ).toBe(true);
...
`;
      ...
      expect(logs.some((log) => log.includes("Files modified: 1"))).toBe(true);
      expect(
        logs.some((log) => log.includes("Link style: converted to markdown")),
      ).toBe(true);
...
`;
      ...
      expect(logs.some((log) => log.includes("Files modified: 1"))).toBe(true);
      expect(
        logs.some((log) => log.includes("Link style: converted to claude")),
      ).toBe(true);
