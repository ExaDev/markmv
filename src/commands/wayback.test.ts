...
function isJsonReport(value: unknown): value is JsonReport {
  ...
  if (
    !("totalConverted" in value) ||
    !("totalUntouched" in value) ||
    !("files" in value)
  ) {
    return false;
  }
...
describe("toWaybackUrl", () => {
  ...
  it("converts http URLs and preserves the query string and fragment verbatim", () => {
    expect(
      toWaybackUrl("http://example.com/docs/page?id=42&sort=asc#section"),
    ).toBe(
  ...
  it("returns undefined for URLs already pointing at the Wayback Machine", () => {
    expect(
      toWaybackUrl("https://web.archive.org/web/2020/https://example.com"),
    ).toBeUndefined();
    expect(
      toWaybackUrl(
        "http://web.archive.org/web/20201001000000*/http://example.com",
      ),
    ).toBeUndefined();
  });
...
describe("Wayback Command", () => {
  ...
  it("converts external http(s) links in a markdown file", async () => {
    ...
    expect(content).toContain(
      "[GitHub](https://web.archive.org/web/*/https://github.com)",
    );
    expect(content).toContain(
      "[the guide](https://web.archive.org/web/*/http://example.com/docs)",
    );
  });
  ...
  it("preserves archived and non-web links and reports per-file counts", async () => {
    ...
    expect(updated).toContain(
      "[live](https://web.archive.org/web/*/https://example.com/live)",
    );
    expect(updated).toContain(
      "[archived](https://web.archive.org/web/2020/https://example.com/old)",
    );
  ...
  it("converts angle-bracket autolinks", async () => {
    ...
    const updated = await readFile(testFile, "utf-8");
    expect(updated).toContain(
      "<https://web.archive.org/web/*/https://example.com/autolink>",
    );
  });
  ...
  it("prints per-file detail and a summary when verbose", async () => {
    ...
    expect(
      logs.some((log) => log.includes("Starting Wayback Machine conversion")),
    ).toBe(true);
    expect(
      logs.some((log) =>
        log.includes("Dry run mode: no files will be modified"),
      ),
    ).toBe(true);
    ...
    expect(logs.some((log) => log.includes("Wayback Machine Summary"))).toBe(
      true,
    );
    ...
    expect(
      logs.some((log) =>
        log.includes("Dry run - no files were actually modified"),
      ),
    ).toBe(true);
  });
  ...
  describe("error handling", () => {
    ...
    beforeEach(() => {
      ...
      process.exit = (code?: number): never => {
        ...
        throw new Error(
          `halted: process.exit called with ${String(code ?? 0)}`,
        );
      };
    ...
    it("exits with code 1 when no patterns are given", async () => {
      ...
      expect(
        errors.some((error) =>
          error.includes("At least one file pattern must be specified"),
        ),
      ).toBe(true);
    });

    it("exits with code 1 when no markdown files match the patterns", async () => {
      await waybackCommand([join(testDir, "non-existent.md")], {}).catch(
        () => undefined,
      );

      expect(exitCalls).toEqual([1]);
      expect(
        errors.some((error) => error.includes("No markdown files found")),
      ).toBe(true);
    });
  });
  ...
  describe("destination precision", () => {
    ...
    it("keeps link titles intact", async () => {
      ...
      const updated = await readFile(testFile, "utf-8");
      expect(updated).toBe(
        '[a](https://web.archive.org/web/*/https://example.com "the title")\n',
      );
    });
