...
`;
      ...
      const internalBrokenLink = brokenLinks.find(
      ...
      const anchorBrokenLink = brokenLinks.find(
...
`;
  ...
  describe("obsidian mode", () => {
    ...
    it("validates wikilinks against the vault and flags unresolved ones", async () => {
      await writeFile(
        ...
        "Good: [[Existing]], bad: [[Missing Note]]\n",
      );
  ...
  describe("standards enforcement", () => {
    ...
    it("reports files missing required frontmatter fields", async () => {
      ...
      const result = await validateLinks(
        [withFrontmatter, withoutFrontmatter],
        {
          ...enforcementOptions,
    ...
    it("reports internal links that violate the enforced link format", async () => {
      ...
      await writeFile(
        sourceFile,
        "[rel](./ok.md) and [abs](/tmp/absolute.md)\n",
      );
  ...
  describe("fix mode", () => {
    const fixOptions: Omit<ValidateOperationOptions, "linkTypes"> & {
      fix: boolean;
    } = {
      checkExternal: false,
      externalTimeout: 5000,
    ...
    it("plans fixes for broken internal links with ranked suggestions", async () => {
      ...
      await writeFile(
        join(testDir, "guides", "getting-started.md"),
        "# Guide\n",
      );
      ...
      expect(fixes[0]?.suggestions[0]?.replacementHref).toBe(
    ...
    it("prints suggestions without prompting when stdout is not a TTY", async () => {
      ...
      expect(await readFile(sourceFile, "utf-8")).toContain(
        "./getting-strated.md",
      );
    });
    ...
    it("applies prompted fixes interactively", async () => {
      ...
      const prompter: FixPrompter = (fix) => {
        expect(fix.suggestions[0]?.replacementHref).toBe(
          "./guides/getting-started.md",
        );
