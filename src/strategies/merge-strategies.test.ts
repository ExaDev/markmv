...
class TestableAppendMergeStrategy extends AppendMergeStrategy {
  ...
  public testDetectTransclusionLoops(
    ...
    return this.detectTransclusionLoops(
      targetFile,
      sourceFile,
      existingTransclusions,
    );
  }

  public testExtractHeaders(
    content: string,
...
Different content for section A.`;
  ...
  describe("AppendMergeStrategy", () => {
    it("should append source content to target", async () => {
      ...
      const targetIndex = result.content.indexOf(
        "This is the target document.",
      );
      const sourceIndex = result.content.indexOf(
        "This is the source document.",
      );
    ...
    it("should detect header conflicts", async () => {
      ...
      expect(result.conflicts.some((c) => c.type === "header-collision")).toBe(
        true,
      );
      expect(
        result.conflicts.some((c) => c.description.includes("Section A")),
      ).toBe(true);
    });
...
More content.`;
      ...
      expect(result.warnings.some((w) => w.includes("loop detected"))).toBe(
        true,
      );
    });
  ...
  describe("InteractiveMergeStrategy", () => {
    it("should create interactive conflicts for manual resolution", async () => {
      ...
      expect(result.warnings.some((w) => w.includes("Interactive merge"))).toBe(
        true,
      );
    });
    ...
    it("should create conflicts for each header placement decision", async () => {
      ...
      const headerConflicts = result.conflicts.filter(
        (c) => c.type === "header-collision",
      );
      const placementConflicts = result.conflicts.filter(
        (c) => c.type === "content-overlap",
      );
...
End.`;
    ...
    it("should create transclusion references", () => {
      expect(strategy.testCreateTransclusion("test-file.md")).toBe(
        "![[test-file]]",
      );
    ...
    it("should detect transclusion loops", () => {
      ...
      expect(
        strategy.testDetectTransclusionLoops(
          "file-a.md",
          "file-c.md",
          existingRefs,
        ),
      ...
      expect(
        strategy.testDetectTransclusionLoops("same.md", "same.md", []),
      ).toBe(true);
    });
