...
/** Local cross-platform testing script Simulates different filesystem behaviors for testing */
...
function detectFilesystemCapabilities() {
  ...
  try {
    ...
    try {
      ...
      capabilities.supportsSpaces = existsSync(
...
function runTests() {
  ...
  testEnv.MARKMV_TEST_SUPPORTS_SYMLINKS =
    capabilities.supportsSymlinks.toString();
  testEnv.MARKMV_TEST_FILESYSTEM_CASE_SENSITIVE =
    capabilities.caseSensitive.toString();
...
function testCLI() {
  ...
  try {
    ...
    if (currentPlatform === "win32") {
      execSync(
        'node dist/cli.js move "test-data/cross-platform/doc1.md" "test-data/cross-platform/moved\\\\doc1.md" --dry-run',
        {
          env: testEnv,
          stdio: "pipe",
        },
