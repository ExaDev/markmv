#!/usr/bin/env node
/** Script to generate API documentation section for README.md from TypeDoc markdown output */
...
function generateApiSection() {
  try {
    ...
    if (!existsSync(MARKDOWN_DOCS_PATH)) {
      console.error(
        "❌ Markdown docs not found. Run `npm run docs:markdown` first.",
      );
      process.exit(1);
    }
    ...
    const hasApiSection =
      currentReadme.includes(API_SECTION_START) &&
      currentReadme.includes(API_SECTION_END);

    if (!hasApiSection) {
      console.log("📝 No API section markers found in README.md");
      console.log(
        "To enable automatic API documentation injection, add these markers to your README.md:",
      );
      console.log("");
      console.log(API_SECTION_START);
      console.log(
        "<!-- Auto-generated API documentation will be inserted here -->",
      );
      console.log(API_SECTION_END);
      console.log("");
      console.log(
        "💡 For now, I'll create a separate api-docs.md file with the generated documentation.",
      );
