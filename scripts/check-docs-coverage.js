...
class DocCoverageChecker {
  ...
  generateReport() {
    ...
    const exportCoverage =
      this.stats.totalExports > 0
        ? (
            (this.stats.documentedExports / this.stats.totalExports) *
            100
          ).toFixed(1)
        : 0;
    ...
    const classCoverage =
      this.stats.totalClasses > 0
        ? (
            (this.stats.documentedClasses / this.stats.totalClasses) *
            100
          ).toFixed(1)
        : 0;
    ...
    const propertyCoverage =
      this.stats.totalProperties > 0
        ? (
            (this.stats.documentedProperties / this.stats.totalProperties) *
            100
          ).toFixed(1)
        : 0;
    ...
    const overallCoverage =
      totalItems > 0 ? ((totalDocumented / totalItems) * 100).toFixed(1) : 0;
      ...
      if (this.stats.issues.length > 0) {
        ...
        for (const [file, issues] of Object.entries(issuesByFile)) {
          ...
          for (const issue of issues) {
            ...
            console.log(
              `   ${icon} Line ${issue.line}: ${issue.construct} '${issue.name}'`,
            );
          }
        }
      } else {
  ...
  run() {
    ...
    if (!isJsonOutput) {
      console.log(
        "🔍 Analyzing TypeScript files for documentation coverage...\n",
      );
    }
...
if (!isJsonOutput) {
  if (result.overallCoverage < threshold) {
    ...
    if (
      ...
      process.env.CI_STRICT_DOCS === "true"
    ) {
      process.exit(1);
    }
  } else {
